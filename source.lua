...
function LunaCore:CreateWindow(configs)
    ...
    -- สร้าง ScreenGui หลักใน CoreGui
    ...
    -- หน้าต่างหลัก (Main Window Frame)
    ...
    -- แถบเมนูด้านซ้าย (Sidebar Container)
    ...
    -- หน้าแสดงฟังก์ชันหลักขวา (Content Containers)
    ...
    -- สวิตช์สลับปิดเปิดด้วยปุ่มคีย์บอร์ด
...
return LunaCore    Title.TextXAlignment = Enum.TextXAlignment.Left
    Title.Parent = Sidebar
    ...
    -- Make UI Dragging (ทำให้หน้าต่างลากย้ายหน้าจอได้)
    local dragging, dragInput, dragStart, startPos
    MainFrame.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            dragging = true; dragStart = input.Position; startPos = MainFrame.Position
    ...
    -- สวิตช์สลับปิดเปิดหน้าต่างด้วยปุ่มบนคีย์บอร์ด
    UserInputService.InputBegan:Connect(function(input, processed)
        if not processed and input.KeyCode == (configs.ToggleKey or Enum.KeyCode.RightControl) then
            ScreenGui.Enabled = not ScreenGui.Enabled
        end
    ...
    -- 🛠️ ฟังก์ชันย่อยสำหรับปั้นวัตถุภายใน Window
    local WindowMethods = {}
    local firstTab = true
    ...
    function WindowMethods:CreateTab(tabConfigs)
        ...
        TabButton.BackgroundTransparency = 1
        TabButton.Text = "  " .. tabName
        TabButton.TextColor3 = Color3.fromRGB(15, 160, 180)
        TabButton.Font = Enum.Font.SourceSansBold
        TabButton.TextSize = 15
        TabButton.TextXAlignment = Enum.TextXAlignment.Left
        TabButton.Parent = TabHolder
        ...
        -- 📄 หน้าเก็บส่วนประกอบขวาของแท็บนั้นๆ
        local TabPage = Instance.new("ScrollingFrame")
        TabPage.Name = "TabPage_" .. tabName
        TabPage.Size = UDim2.new(1, 0, 1, 0)
        TabPage.BackgroundTransparency = 1
        ...
        -- 🛠️ ฟังก์ชันการสร้างวัตถุควบคุมภายในแต่ละหน้าแท็บ (Buttons, Toggles, Sliders)
        local TabMethods = {}

        -- 1. สร้างปุ่มธรรมดา (Button)
        function TabMethods:CreateButton(btnConfigs)
            local btnName = btnConfigs.Name or "Button"
            local callback = btnConfigs.Callback or function() end
        ...
        -- 3. สร้างแถบสไลด์ปรับระดับ (Slider)
        function TabMethods:CreateSlider(sliderConfigs)
            local sliderName = sliderConfigs.Name or "Slider"
            local min = sliderConfigs.Range[1] or 0
            local max = sliderConfigs.Range[2] or 100
